...
本文件只写每个会话都必须生效的规则；操作细节在对应 skill（见文末路由表）。
...
写死的例子（真源在测试里）：退出码、状态目录文件名、`dsh web:` 地址行格式、Node 下限 `24.12.0`、进程归属 = 运行记录 + 启动时间指纹、轮转截断同一 inode、`latest` = `origin/master`。可配置的例子：`repoDir`、`port`、`nodeVersion`、三个超时、`logRotateBytes`。
