...
`internal/service`）也在加英文信息，`internal/repo/prune_audit_test.go` 里被 skill 引用为
...
按规则写了中文信息，另一个 agent 在 `internal/service` 也写了中文，评审时必须先判断
...
- 已知缺口：约 35 条历史中文失败信息留在树里（多在 `internal/service`、`internal/config`、
...
- 本次会话新增/改动的失败信息已全部改为英文：`internal/cli/documentation_test.go`、
  ...
  `internal/repo/prune_test.go`、`internal/service/lock_test.go`、
  `internal/service/tolerance_test.go`。
