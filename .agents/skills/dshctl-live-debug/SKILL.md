...
1. `dshctl status --json`：看 `state`（`running`/`starting`/`stopped`/`port-foreign`/`port-unmanaged`，定义见 `../../../internal/service/observe.go`）、`ready`、`listenerPid` 与 `recordedPid` 是否一致、`recordLive`/`recordStale`/`survivorService`、以及 `recordedRepoDir`/`recordedNodeVersion`——"运行中的服务"与"配置里的 checkout"可能不是同一个，前者以记录为准。
...
- dshctl 只在运行记录里的 pid **与该进程的启动时间仍然吻合**时才结束它，因为 pid 会被操作系统回收，启动时间指纹是唯一的归属证据（`../../../internal/service/service.go` 的三条不变量、`../../../internal/state/state.go` 的记录模型）。
...
- `../../../internal/service/observe.go`：状态取值与 `status --json` 的字段语义。
- `../../../internal/service/service.go`：进程归属、"探测不了不等于没有"、不误杀他人服务三条不变量。
