...
11. **记录按端口分文件，跨端口守卫用转义过的 glob。** 为什么：`build`/`update` 会 glob 出其他端口的记录以免误停别人的服务，而状态目录路径可能含 `[`、`*`、`?`——未转义的 glob 匹配不到任何东西，守卫就瞎了（`internal/config/config.go` 的 `StateFileGlob`/`quoteGlob`，`internal/app/contracts_test.go` 把它钉住）。
...
- 跨端口守卫：[../../../internal/app/contracts_test.go](../../../internal/app/contracts_test.go)
