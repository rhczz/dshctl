...
6. **体检与运行记录也是表面。** `doctor` 每一项是 `internal/app/doctor.go` 的 `Check{Name, Status, Detail}`（中文短标签，`Detail` 给观测值或补救动作）；`status --json` 与 `url` 读 `internal/state/state.go` 的 `Record`。为什么：排障时操作者只看这两处，新增字段却不进记录或体检，等于让 README 的运行记录一节说谎。
...
10. **文案简洁、说明后果、给出下一步。** 例："仓库目录 … 不存在；用 --repo 或环境变量 DSH_REPO_DIR 指定一次，成功运行后会写入 …"。为什么：操作者与 agent 都照这句话决定下一步动作，只说"失败"会逼人去读源码（`internal/app/doctor.go` 的失败行即范例）。
...
- 体检与记录：[../../../internal/app/doctor.go](../../../internal/app/doctor.go)、[../../../internal/state/state.go](../../../internal/state/state.go)
