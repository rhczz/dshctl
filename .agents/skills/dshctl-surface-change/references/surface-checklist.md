...
| 新增/修改 `doctor` 检查项 | `internal/service/doctor.go` 的 `Check{Name, Status, Detail}` 列表 | 涉及新配置或新文件时同步 README 对应表 | `go test ./internal/service/ -run 'TestDoctor' -count=1` |
| 新增/修改运行记录字段 | `internal/state/state.go` 的 `Record`（json tag） | `README.md` 状态目录一节的记录说明 | `go test ./internal/state/ ./internal/service/ -count=1` |
