...
| `make coverage` | **CI**：改 `internal/nodejs` 或配置层决策 | hermetic 跑一次带 `-coverprofile`，再交 `../../../../scripts/check-coverage.py` | `internal/nodejs` 100% 硬门禁；`internal/config`、`internal/kernel` 只报告 | `FAIL internal/nodejs: xx.x% (要求 100%，a/b 条语句)` 加 `未覆盖:` 行 |
...
14. `检查失败: go-imports: <file>: <包> 依赖 <包>，不在允许的层方向内` → 依赖方向反了或新增了跨层边 → 把逻辑放回上层，或经 `internal/kernel` 中转（`dshctl-decisions`）。
