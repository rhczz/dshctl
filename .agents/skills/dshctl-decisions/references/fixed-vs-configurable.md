...
| `dsh web: <url>` 地址行格式 | 固定：外部规范 | 这是被管理服务打印的格式，不是 dshctl 的选择；解析正则在 `internal/service/start.go` |
| 构建标记 `.dsh-build/client-build-environment.json` | 固定：跨进程契约 | 构建写、`start` 与 `doctor` 读，三个包必须一致，由 `internal/service/contracts_test.go` 钉住 |
...
| "探测不了"不当作"没有" | 固定：安全不变量 | 把探测失败当成"端口空闲"会启动第二个服务或误杀；`internal/host` 与 `internal/service` 各有一条 |
