...
7. **依赖方向不可逆**：`cmd/dshctl → internal/cli → internal/app → 叶子`，跨层调用只经 `service`。系统调用只出现在 `host` 以及 `run`/`detach`/`logfile`/`atomically`/`lock`/`state`/`cmd/dshctl` 的平台文件里，上层不得出现 `if windows`。新增一条 import 边是设计决定，不是实现细节；`scripts/check-conventions.py` 会拦下未登记的边。

8. **接口只为可测性存在。** 当前全仓库只有四个接口：`run.Executor`/`Capturer`/`Outputer` 与 `app.OsHost`。它们存在，是因为那样就能把整个生命周期跑在虚构机器上。新增接口时，文档必须写明它买到了什么测试能力；"将来可能换实现"不是理由。
...
- [`internal/app/host.go`](../../../internal/app/host.go)：`OsHost` 接口及其"为虚构机器而存在"的文档
