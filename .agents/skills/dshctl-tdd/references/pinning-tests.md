...
3. **交给 `make mutation`**。改动 `internal/nodejs` 或配置层决策时不必手工做：`scripts/mutation-check.py` 会逐条做字面替换、跑相关包、要求失败、再还原。它当前覆盖 35 条决策（权威清单是 `python3 scripts/mutation-check.py --list`），其中包括：
