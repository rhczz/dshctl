...
4. **跨进程与多态行为先在虚构机器上写失败测试。** `start`/`stop`/端口归属/信号这类行为不要一上手就起真实进程：`internal/service/fake_test.go` 的虚构进程表（含 `survivesGraceful`、`survivesForce` 这类只在异常路径才用到的形状）、`internal/service/host.go` 的 `OsHost`、`internal/run` 的 `Executor` 就是为此存在的。虚构机器先证明决策逻辑；真实进程测试（`internal/host/realps_test.go`、`internal/repo/prune_test.go`）再证明它与真实世界对得上。
...
- [`internal/service/fake_test.go`](../../../internal/service/fake_test.go)：虚构进程表与它的异常路径形状
- [`internal/service/contracts_test.go`](../../../internal/service/contracts_test.go)：跨包字面量的契约钉法
