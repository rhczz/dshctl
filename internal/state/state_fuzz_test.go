...
// fingerprint all decide whether a server can still be managed.
func FuzzSaveLoadRoundTrip(f *testing.F) {
	f.Add(1, 2, int64(1_700_000_000), 3080, "running", "http://127.0.0.1:3080/?token=abc")
	f.Add(48737, 48736, int64(0), 65535, "", "")
	f.Add(2, 0, int64(-1), 1, "stopping", "\n")
	f.Add(3, 3, int64(9_999_999_999), -1, "running", "标题")

	f.Fuzz(func(t *testing.T, pid, spawnedPID int, startedAt int64, port int, phase, url string) {
		...
		if !utf8.ValidString(phase) || !utf8.ValidString(url) {
			return
		}
		...
		if loaded.PID != saved.PID || loaded.SpawnedPID != saved.SpawnedPID ||
			loaded.StartedAt != saved.StartedAt || loaded.Port != saved.Port ||
			loaded.Phase != saved.Phase || loaded.URL != saved.URL {
