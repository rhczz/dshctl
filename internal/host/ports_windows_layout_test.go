...
// MIB_TCP6ROW_OWNER_PID (56 bytes) reads fields out of the middle of another
...
// invisible in review: the IPv4 row is 24 bytes and the IPv6 row looks like the
// same field list with wider addresses, while it really carries a remote scope
// id as well.
func TestTCPTableRowSizes(t *testing.T) {
	...
	if got := unsafe.Sizeof(mibTCP6Row{}); got != 56 {
		t.Fatalf("mibTCP6Row is %d bytes, want the 56 of MIB_TCP6ROW_OWNER_PID", got)
	}
