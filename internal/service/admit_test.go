...
func TestOccupantIsExactlyWhatNoVerbMayActOn(t *testing.T) {
	cases := []struct {
		...
		{"running", observed{status: domain.Status{State: domain.StateRunning}}, false},
		{"starting", observed{status: domain.Status{State: domain.StateStarting}}, false},
		{"stopped", observed{status: domain.Status{State: domain.StateStopped}}, false},
		{"unobservable", observed{status: domain.Status{State: domain.StateUnobservable}}, false},
		{"foreign", observed{status: domain.Status{State: domain.StateForeign}}, true},
		{"orphan", observed{status: domain.Status{State: domain.StateOrphan}}, true},
		{"survivor", observed{status: domain.Status{State: domain.StateOrphan, Survivor: true}}, false},
	}
...
func TestAdmitSurvivorAdoptsAndReobserves(t *testing.T) {
	...
	if before.status.State != domain.StateOrphan || !before.status.Survivor {
	...
	if after.status.State != domain.StateRunning || !after.status.RecordLive {
