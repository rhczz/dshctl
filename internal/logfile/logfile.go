...
func (l *Logger) RotateIfNeeded() (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("the size of %s could not be read: %w", l.Path, err)
	}
	if !info.Mode().IsRegular() {
		return false, fmt.Errorf("the log path %s is not a regular file; check DSH_LOG_FILE", l.Path)
	}
	...
	if err := os.Truncate(l.Path, 0); err != nil {
		return false, fmt.Errorf("rotation failed: %s could not be truncated: %w", l.Path, err)
	}
	return true, nil
}
...
func (l *Logger) copyToBackupStable() error {
	for {
		info, err := os.Stat(l.Path)
		if err != nil {
			return fmt.Errorf("rotation failed: %s could not be read: %w", l.Path, err)
		}
...
func (l *Logger) copyToBackup(size int64) error {
	...
	if err != nil {
		return fmt.Errorf("rotation failed: %s could not be written: %w", l.BackupPath(), err)
	}
	written, copyErr := io.CopyBuffer(backup, io.LimitReader(source, size), make([]byte, copyBufferBytes))
	syncErr := backup.Sync()
	closeErr := backup.Close()
	if copyErr != nil {
		return fmt.Errorf("rotation failed while copying to %s: %w", l.BackupPath(), copyErr)
	}
	if syncErr != nil {
		return fmt.Errorf("rotation failed while syncing %s: %w", l.BackupPath(), syncErr)
	}
	if closeErr != nil {
		return fmt.Errorf("rotation failed while closing %s: %w", l.BackupPath(), closeErr)
	}
	if written != size {
		return fmt.Errorf("rotation failed: %s received only %d/%d bytes", l.BackupPath(), written, size)
	}
	return nil
}
...
func (l *Logger) Size() (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("the size of %s could not be read: %w", l.Path, err)
	}
...
func (l *Logger) open() (*os.File, error) {
	if err := os.MkdirAll(filepath.Dir(l.Path), 0o700); err != nil {
		return nil, fmt.Errorf("the log directory %s could not be created: %w", filepath.Dir(l.Path), err)
	}
	file, err := os.OpenFile(l.Path, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600)
	if err != nil {
		return nil, fmt.Errorf("the log %s could not be opened: %w", l.Path, err)
	}
	return file, nil
}
...
func (l *Logger) append(text string) error {
	...
	if _, err := file.WriteString(text); err != nil {
		return fmt.Errorf("the log %s could not be written: %w", l.Path, err)
	}
	return nil
}
...
func ValidateTitle(title string) error {
	if title == "" {
		return errors.New("a section title cannot be empty")
	}
	if index := strings.IndexFunc(title, unicode.IsSpace); index >= 0 {
		return fmt.Errorf("a section title cannot contain whitespace: %q", title)
	}
	return nil
}
...
func TailFrom(path string, lines int, w io.Writer) (int, int64, error) {
	...
	if _, statErr := os.Stat(path); errors.Is(statErr, fs.ErrNotExist) {
		...
		return 0, 0, fmt.Errorf("the size of %s could not be read: %w", path, statErr)
	}
...
func readTailLines(path string, count int, limit int64) ([]byte, int64, error) {
	...
	info, err := file.Stat()
	if err != nil {
		return nil, 0, fmt.Errorf("the size of %s could not be read: %w", path, err)
	}
	if !info.Mode().IsRegular() {
		...
		return nil, 0, fmt.Errorf("the log path %s is not a regular file; check DSH_LOG_FILE", path)
	}
	size := info.Size()
	if size == 0 {
		return nil, 0, nil
	...
	for position > 0 {
		...
		if _, err := file.ReadAt(chunk, position); err != nil && !errors.Is(err, io.EOF) {
			return nil, 0, fmt.Errorf("%s could not be read: %w", path, err)
		}
...
func readTailBytes(path string, limit int64) ([]byte, bool, error) {
	...
	info, err := file.Stat()
	if err != nil {
		return nil, false, fmt.Errorf("the size of %s could not be read: %w", path, err)
	}
	if !info.Mode().IsRegular() {
		return nil, false, fmt.Errorf("the log path %s is not a regular file; check DSH_LOG_FILE", path)
	}
	offset := int64(0)
	truncated := false
	if info.Size() > limit {
	...
	if _, err := file.Seek(offset, io.SeekStart); err != nil {
		return nil, false, fmt.Errorf("%s could not be located: %w", path, err)
	}
	data, err := io.ReadAll(file)
	if err != nil {
		return nil, false, fmt.Errorf("%s could not be read: %w", path, err)
	}
