...
- 只含空白的变量视为未设置。
- 面向操作者的文案默认英文，机器语言是中文时用中文：`DSHCTL_LANG` 优先于 shell 导出的 locale 变量，认不出的语言按英文处理（不会出现半翻译的界面）。帮助、错误、状态与体检的每一句话都在各层的消息目录里，新增文案必须进目录——`make conventions` 的 `i18n-literals` 规则会拒绝写在代码里的中文文案。
